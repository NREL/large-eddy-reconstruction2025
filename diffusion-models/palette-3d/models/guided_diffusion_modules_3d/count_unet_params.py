...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        ## Operations that come up if we are upscaling/downscaling
        ...
        self.out_layers = nn.Sequential(
            ...
            ),  # zero_module info: https://discuss.pytorch.org/t/why-would-someone-zero-out-the-parameters-of-a-module-in-the-constructor/157148
        )

        # Define what the skip connection will look like
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        ## ~~~~~ Input blocks ~~~~~
        # Set the integer size of ch and input_ch
        ch = input_ch = int(channel_mults[0] * inner_channel)  # Chained assignment for ch and input_ch

        ## Initialize the module list
        self.input_blocks = nn.ModuleList(
            [EmbedSequential(nn.Conv3d(in_channel, ch, 3, padding=1))]
        )
        self._feature_size = ch  # _feature_size doesn't appear to be used anywhere
        input_block_chans = [ch]  # Keeps track of ch at each layer; to be later used by the output blocks
        ds = 1  # An iterator that gets multiplied/divided by 2; associated with attention

        ## Add layers to the input section
        for level, mult in enumerate(channel_mults):
            for _ in range(res_blocks):  # Iterate over the number of res blocks per downsample (~2)
                layers = [
                    ResBlock(
                        ch,
                        cond_embed_dim,
            ...
            if level != len(channel_mults) - 1:  # If not the innermost level
                out_ch = ch
...
if __name__ == '__main__':
    ...
    print("Countable parameters: ", sum(p.numel() for p in model.parameters() if p.requires_grad))
